...
def setup_slice_sampler(log_pdf, D, S, num_chains=1):
    """This function takes as input the log pdf, parameters.
        ...
        The returned function generates `S` samples from `num_chains` 
        number of chains given initial point `x0` and parameters `theta`."""
    ...
    def forwards_step(x, theta, u1, u2, d):#, aL, bR):
        """Takes one step forward in slice sampling."""
    ...
    def forwards_sample(theta, x0, key):
        """Generates S samples from slice sampling using fori_loop."""
        # generate randomness
        # TODO: generate randomness outside of forward sample 
    ...
    def backwards_step(theta, dL_dtheta, us, d, x, xL, xR, alphas, dL_dx, prev_dL_dx):
        ...
        prev_dL_dx is the gradient the loss wrt x_{s+1}.""" 
    ...
    def backwards(S, theta, us, ds, xs, xLs, xRs, alphas, dL_dxs):
        ...
        def body_fun(val):
            ...
            dL_dtheta, prev_dL_dx = backwards_step(theta, dL_dtheta, us[s,:], ds[s], xs[s], xLs[s], xRs[s], alphas[s], dL_dxs[s], prev_dL_dx)
            val[0] -= 1z
    ...
    def slice_sample_bwd(res, g):
        ...
        # grad_theta needs to be size of theta
        # grad_x0 needs to be size of x0

        # --- unpack results of `slice_sample_fwd`
        forwards_out, theta = res
        xs0, us, ds, xLs, xRs, alphas = forwards_out

        grad_thetas, grad_x0 = vmapped_backwards(S, theta, us, ds, xs0, xLs, xRs, alphas, g)
