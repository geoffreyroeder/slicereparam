...
def negative_elbo(zs, xs, params):
    ...
    out = np.mean(-0.5 * (zs - mu[None,:])**2) # p(z) = N(mu, I)
    params = stop_gradient(params)
    out = out + np.mean(vmap_gaussian_log_pdf(xs, zs, np.eye(D))) # p(x|z) = N(z, I)
    out = out - np.mean(vmapped_log_pdf(zs, params, xs)) # entropy term (grad part)
    return -1.0 * out
...
def loss_slice(params, z0, xs, key):
    ...
    out = np.mean(np.sum(-0.5 * (zs - mu[None,:])**2, axis=1)) # p(z) = N(mu, I)
    params = stop_gradient(params)
    out = out + np.mean(vmap_gaussian_log_pdf(xs, zs, np.eye(D))) # p(x|z) = N(z, I)
    out = out - np.mean(vmapped_log_pdf(zs, params, xs)) # entropy term (grad part)
    return -1.0 * out 
